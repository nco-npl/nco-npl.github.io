...
function calculateConstrainedPathParameters(sourceItem, targetItem) {
  ...
  if (availableGap < 30) {
    return { path: 'straight', pathOptions: {} };
  }
  return {
    path: 'arc',
    pathOptions: {
...
function checkMatch(sourceItem, targetItem) {
  if (sourceItem.dataset.match === targetItem.id) {
    ...
    if (!isAlreadyMatched) {
      ...
      existingLines.push({ start: sourceItem, end: targetItem, line });
...
function isItemMatched(item) {
  return existingLines.some(line => line.start === item || line.end === item);
}

function getMatchedPair(item) {
  const match = existingLines.find(line => line.start === item || line.end === item);
...
function selectItem(item) {
  ...
  if (isItemMatched(item)) {
    ...
    announce(`${item.getAttribute('aria-label')} is already matched with ${matchedItem.getAttribute('aria-label')}. Please select an unmatched item.`);
    return false;
  }
  selectedItem = item;
...
document.querySelectorAll('.item').forEach(item => {
  ...
  item.addEventListener('focus', () => {
    if (isItemMatched(item)) {
      const matchedItem = getMatchedPair(item);
      announce(`${item.getAttribute('aria-label')} is already matched with ${matchedItem.getAttribute('aria-label')}`);
    }
  });
  ...
  item.addEventListener('keydown', e => {
    switch (e.key) {
      ...
      case ' ':
        ...
        if (!selectedItem) {
          selectItem(item);
          const otherGroup = item.parentElement.id === 'left-items' ? rightContainer : leftContainer;
          const unmatchedItems = Array.from(otherGroup.children).filter(i => !isItemMatched(i));
          if (unmatchedItems.length > 0) unmatchedItems[0].focus();
          else announce('No unmatched items remaining in the other group.');
        } else if (selectedItem === item) {
          clearSelection();
        } else {
          attemptMatch(selectedItem, item);
      ...
      case 'ArrowDown':
        ...
        const nextIndex = e.key === 'ArrowUp'
          ? (items.indexOf(item) - 1 + items.length) % items.length
      ...
      case 'ArrowRight':
        e.preventDefault();
        const targetContainer = item.parentElement.id === 'left-items' ? rightContainer : leftContainer;
        const sourceRect = item.getBoundingClientRect();
        let closestItem = targetContainer.children[0];
        let minDistance = Infinity;
        Array.from(targetContainer.children).forEach(t => {
          const dist = Math.abs(t.getBoundingClientRect().top - sourceRect.top);
          if (dist < minDistance) {
            minDistance = dist;
            closestItem = t;
          }
  ...
  item.addEventListener('dragover', e => {
    e.preventDefault();
    if (dragged !== item) item.setAttribute('aria-dropeffect', 'move');
  });
